...
bmp_image bmp_read(char *bmp_path)
{
    ...
    for (i = 0; i < image.header.height; i++) {
        for (j = 0; j < image.header.width; j++) {
            ...
            color = (double) fgetc(bmp_file) / 255;
            image.pixel_array[i * image.header.width + 2 * size + j] =
                color;
        }
...
void bmp_write(char *bmp_path, bmp_image image, double *pixel_array2)
{
    FILE *bmp_file;
    int i, j, k;
    ...
    if (fwrite(bmp_header_buf, 1, BMP_HEADER_SIZE, bmp_file) <
        BMP_HEADER_SIZE) {
    ...
    for (i = 0; i < image.header.height; i++) {
        for (j = 0; j < image.header.width; j++) {
            double color;
            /*( Считываем значение синего компонента цвета пикселя */
            color = pixel_array2[i * image.header.width + j];
            fputc((uint8_t) (color * 255), bmp_file);
            /* Считываем значение зеленего компонента цвета пикселя */
            color = pixel_array2[i * image.header.width + size + j];
            fputc((uint8_t) (color * 255), bmp_file);
            /* Считываем значение красного компонента цвета пикселя */
            color = pixel_array2[i * image.header.width + 2 * size + j];
            fputc((uint8_t) (color * 255), bmp_file);
        }
